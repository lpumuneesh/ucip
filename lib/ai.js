...
export async function generateDailyIntel({ changes, universities }) {
  // Group changes by university/type
  const byUni = {}
  for (const c of changes) {
    const code = c.universityCode
    if (!byUni[code]) byUni[code] = []
    byUni[code].push(c)
  }
  const changesSummary = Object.entries(byUni).map(([code, arr]) => {
    const uniName = arr[0]?.universityName || code
    const lines = arr.slice(0, 8).map(c => {
      const what = c.type
      let detail = ''
      if (c.before !== undefined && c.after !== undefined) {
        detail = ` — was "${String(c.before || '').slice(0,80)}" now "${String(c.after || '').slice(0,80)}"`
      } else if (c.added?.length || c.removed?.length) {
        const added = (c.added || []).slice(0, 3).join(', ')
        const removed = (c.removed || []).slice(0, 3).join(', ')
        detail = ` — ${added ? `+ ${added}` : ''}${removed ? ` − ${removed}` : ''}`
      }
      return `  [${c.severity}] ${c.page || 'site'}: ${what}${detail}`
    }).join('\n')
    return `${uniName} (${code}) — ${arr.length} changes\n${lines}`
  }).join('\n\n')

  const prompt = `You are the Chief Digital Officer's daily briefer for LPU (Lovely Professional University, lpu.in).
Below are competitor website changes detected in the last 24 hours. Your job: extract what LPU MUST do TODAY to stay ahead.

=== COMPETITOR MOVES IN LAST 24 HOURS ===
${changesSummary || 'No changes detected in last 24 hours.'}

Return strict JSON:
{
  "headline": string,                 // one-line summary of the competitive picture today
  "competitiveMoves": [               // 3-6 specific moves competitors made — what & why it matters
    { "competitor": string, "move": string, "whyItMatters": string, "urgency": "critical"|"high"|"medium"|"low" }
  ],
  "todayForLpu": [                    // EXACTLY 3 concrete actions LPU should ship TODAY
    { "action": string, "detail": string, "owner": "Marketing"|"Admissions"|"SEO"|"Content"|"Product"|"Design", "expectedImpact": string, "deadline": "today"|"this week" }
  ],
  "riskSignals": string[],            // 2-4 emerging threats to watch
  "opportunities": string[]           // 2-4 openings LPU can exploit right now
}

Be specific. Reference the actual competitor and their move. If no changes exist, still produce a briefing highlighting LPU's own posture and 3 evergreen daily actions.`

  const res = await client().chat.completions.create({
    model: 'gpt-5',
    messages: [
      { role: 'system', content: 'You are a sharp daily competitive intelligence briefer. Output valid JSON only.' },
      { role: 'user', content: prompt },
    ],
    response_format: { type: 'json_object' },
  })
