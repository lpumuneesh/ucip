...
export function diffSnapshots(prev, curr) {
  ...
  // Page-set changes (new/removed pages) — include per-key URLs
  ...
  if (newPages.length) {
    ...
    changes.push({ type: 'new_pages', page: 'site', severity: 'high', added: newPages, removed: [], addedUrls });
  }
  if (removedPages.length) {
    ...
    changes.push({ type: 'removed_pages', page: 'site', severity: 'high', added: [], removed: removedPages, removedUrls });
  }
