...
export function diffSnapshots(prev, curr) {
  ...
  // Snapshot of key content for a single page — used as evidence when a page is added/removed
  const pageContent = (p) => {
    if (!p) return null;
    return {
  ...
  // Page-set changes (new/removed pages) — include per-key URLs AND content snapshots
  ...
  if (newPages.length) {
    ...
    for (const k of newPages) {
      const p = currPages[k];
      if (p && (p.finalUrl || p.url)) addedUrls[k] = p.finalUrl || p.url;
      const snap = pageContent(p);
      if (snap) addedContents[k] = snap;
    }
    changes.push({ type: 'new_pages', page: 'site', severity: 'high', added: newPages, removed: [], addedUrls, addedContents });
  }
  if (removedPages.length) {
    const removedUrls = {};
    const removedContents = {};
    for (const k of removedPages) {
      const p = prevPages[k];
      if (p && (p.finalUrl || p.url)) removedUrls[k] = p.finalUrl || p.url;
      const snap = pageContent(p);
      if (snap) removedContents[k] = snap;
    }
    changes.push({ type: 'removed_pages', page: 'site', severity: 'high', added: [], removed: removedPages, removedUrls, removedContents });
  }
